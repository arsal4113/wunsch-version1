...
<div id="mouse-dummy"></div>
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-sm-5">
        <h2><?= __('Edit Pillar Page') ?></h2>
        <ol class="breadcrumb">
    ...
    <div class="col-sm-7">
        <div class="title-action">
            <div class="btn-group btn-group-justified btn-actions">
                <div class="btn-group">
                    <?= $this->Html->link('<i class="fa fa-list"></i>' . ' ' . __('List of Pillar Pages'), ['action' => 'index'], ['class' => 'btn btn-sm btn-primary', 'escape' => false]) ?>
                </div>
                <div class="btn-group">
                    <?=
                    $this->Form->postLink(
                        __('Delete'),
                        ['action' => 'delete', $feederPillarPage->id],
                        ['class' => 'btn btn-sm btn-danger', 'confirm' => __('Are you sure you want to delete # {0}?', $feederPillarPage->id)]
                    )
                    ?>
                </div>
            </div>
...
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="ibox">
        <div class="ibox-content">
            <div class="row">
                <div class="col-lg-12">
                    <?= $this->Form->create($feederPillarPage, ['class' => 'form-horizontal style-form', 'type' => 'file']); ?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="card-img-wrapper col-sm-2">
                            <?= ($file = $feederPillarPage->guide_image) ?
                                $this->Html->link(
                                    $this->Html->image($file, ['style' => 'max-width:100%']),
                                    $this->Url->image($file),
...
<script>
    $(document).ready(function () {
        ...
        blockOverview.on('click', '.remove-block', function () {
        ...
        blockOverview.on('mousedown', '.block-wrapper', function (e) {
            if(e.which === 1){
                draggedBlock = $(this);
        ...
        $(window).mousemove(function (e) {
            if(draggedBlock){
                if(dummy.style.display !== 'block'){
                    ...
                    if(draggedBlock.hasClass('small-block')){
                        dummy.classList.add("small-block");
                    }
                }
                dummy.style.top =  e.pageY + "px";
                dummy.style.left =  (e.pageX - sidebarWidth) + "px";
                if(selectedBlock){
                    unselectBlock(selectedBlock);
                }
            }
        });
        ...
        $(window).on('mouseup', function () {
            if(draggedBlock){
                ...
                dummy.classList.remove("small-block");
                let newPosition = parseInt($('.drop-zone.selected').attr('data-position'));
                if(!isNaN(newPosition)){
                    changeDesignOrder(draggedBlock.attr('data-id'), newPosition, checkForCombinedReorder(newPosition));
                }
        ...
        blockOverview.on('mouseover', '.drop-zone', function () {
            if(draggedBlock){
                $(this).addClass('selected')
            }
        });

        blockOverview.on('mouseout', '.drop-zone', function () {
            if(draggedBlock){
                $(this).removeClass('selected')
            }
        });

        function changeDesignOrder(oldIndex, newIndex, sibling) {
            ...
            if(sibling){
                let siblings = sibling === "next" ? designConfig.splice(oldIndex, 2) : designConfig.splice(oldIndex - 1, 2);
                if(newIndex > oldIndex) newIndex--;
                designConfig.splice(newIndex, 0, siblings[0], siblings[1]);
            }else{
                designConfig.splice(newIndex, 0, designConfig.splice(oldIndex, 1)[0]);
            }
            renderBlock();
        }

        function checkForCombinedReorder(newPos){
            let elementId = parseInt(draggedBlock.attr('data-id')),
                draggedElement = designConfig[elementId];

            if(draggedElement.hasSibling){
                if(draggedElement.hasSibling === "next"){
                    if(elementId === newPos - 2){
                        designConfig[elementId].hasSibling = "previous";
                        designConfig[elementId + 1].hasSibling = "next";
                        return false;
                    }
                    return "next";
                }else{
                    if(elementId === newPos + 1){
                        designConfig[elementId].hasSibling = "next";
                        designConfig[elementId - 1].hasSibling = "previous";
                        return false;
                    }
                    return "previous";
                }
