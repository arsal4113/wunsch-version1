    ...
    <div class="col-sm-7">
        <div class="title-action">
            <div class="btn-group btn-group-justified btn-actions">
                <div class="btn-group">
                    <?= $this->Html->link('<i class="fa fa-list"></i>' . ' ' . __('List of Quizzes'), ['action' => 'index'], ['class' => 'btn btn-sm btn-primary', 'escape' => false]) ?>
                </div>
                <div class="btn-group">
                    <?=
                    $this->Form->postLink(
                        __('Delete'),
                        ['action' => 'delete', $feederQuiz->id],
                        ['class' => 'btn btn-sm btn-danger', 'confirm' => __('Are you sure you want to delete # {0}?', $feederQuiz->id)]
                    )
                    ?>
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        ...
        if(questionConfigInput.val() !== ''){
            questionConfig = JSON.parse(questionConfigInput.val());
            renderBlocks();
        }else{
            questionConfig = [{
                question: '',
        ...
        answerContainer.on('click', '.remove-answer', function () {
            ...
            if(currentQuestion.answers.length > 2){
                ...
                currentQuestion.answers.splice(removeId, 1);
                renderAnswers();
            }
        });
