<h3 class="margin-bottom-20 forgot-password-text text-center white"><?= __('Did You forget Your password?') ?></h3>
<?= $this->Form->create(null, ['class' => 'm-t', 'role' => 'form']) ?>
<div class="form-group">
    <?= $this->Form->input('email', ['label' => false, 'class' => 'form-control', 'placeholder' => __('Please type your email here...'), 'autofocus', 'required']) ?>
</div>
<?= $this->Form->button('<i class="fa fa-paper-plane"></i> ' . __('Send request'), ['id' => 'login-button', 'class' => 'btn btn-primary block full-width m-b']); ?>
